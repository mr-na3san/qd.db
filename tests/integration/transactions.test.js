...
describe('Transactions - Cache Consistency', () => {
  ...
  test('should maintain cache consistency on commit', async () => {
    const testDB2 = path.join(__dirname, 'test-transactions-cache.db');
    if (fs.existsSync(testDB2)) {
      fs.unlinkSync(testDB2);
    }
    const db2 = new QuantumDB(testDB2, { cache: true, cacheSize: 100 });
    await db2.set('key1', 'value1');
    expect(await db2.get('key1')).toBe('value1');
    await db2.transaction(async (tx) => {
      tx.set('key1', 'modified');
    });
    expect(await db2.get('key1')).toBe('modified');
    await db2.destroy();
    if (fs.existsSync(testDB2)) {
      fs.unlinkSync(testDB2);
    }
